    ...
    def test_bad_config_remote_path_to_scan_script_raises_exception(self):
        ...
        self.assertEqual(
            Localization.Error.REMOTE_SERVER_INSTALL.format(
                str.join("/usr/bin/scp: dest open \"<bad>/scanfs\": No such file or directory /usr/bin/scp: failed to upload file ", self.context.config.lftp.local_path, " to <bad>/scanfs")
            ),
            str(error.exception)
        )
